...
class Cart extends db_connection
{
    ...
    public function addToCart($product_id, $qty)
    {
        ...
        $check_query = "SELECT qty FROM cart WHERE product_id = ?";
        ...
        if ($result->num_rows > 0) {
            ...
            $update_query = "UPDATE cart SET qty = ? WHERE product_id = ?";
            $stmt = $this->db->prepare($update_query);
            $stmt->bind_param("ii", $new_qty, $product_id);
            return $stmt->execute();
        } else {
            // Add new product to cart
            $insert_query = "INSERT INTO cart (product_id, qty) VALUES (?, ?)";
    ...
    {
        $query = "UPDATE cart SET qty = ? WHERE product_id = ?";
    ...
    /**
     * Retrieve all cart items (joins with products for display).
     */
    public function getCart()
    {
        $query = "SELECT c.cart_id, c.product_id, c.qty, 
     ...
     */
    public function existingProductCheck($product_id)
    {
